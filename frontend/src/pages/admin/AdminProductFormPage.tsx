...
export default function AdminProductFormPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = {
      ...
      minOrderQty: Number(form.minOrderQty), priceRange: form.priceRange,
    };
    try {
      if (isEdit && id) {
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/5 rounded-xl border border-sidebar-border p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea name="description" placeholder="Describe the product..." value={form.description} onChange={handleChange} rows={4} required className="bg-white/5 border-sidebar-border text-sidebar-foreground placeholder:text-sidebar-foreground/30" />
          </div>
