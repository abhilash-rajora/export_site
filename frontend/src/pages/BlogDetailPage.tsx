  ...
  return (
    <div className="mt-6 rounded-2xl overflow-hidden shadow-lg bg-black">
      <div className="relative aspect-video">
        ...
        {all.length > 1 && (
          <>
            <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center">
      ...
      {all.length > 1 && (
        <div className="flex gap-2 p-3 bg-black/80 overflow-x-auto" style={{ scrollbarWidth: 'none' }}>
          {all.map((src, i) => (
            <button key={i} onClick={() => setActive(i)}
              ...
              {isVid(src)
                ? <div className="w-full h-full bg-gray-700 flex items-center justify-center text-white/60 text-xs">▶</div>
        ...
        {open && (
          <motion.div
            ...
            <ol className="space-y-1">
              {headings.map((s, i) => (
                <li key={i}>
                  <a href={`#section-${i}`} onClick={close}
...
const RecentPost = memo(function RecentPost({ post }: { post: Blog }) {
  return (
    <Link to="/blog/$slug" params={{ slug: post.slug }} className="flex gap-3 group">
...
export default function BlogDetailPage() {
  ...
  // Only fetch recent posts once blog data is ready (avoid parallel waterfalls)
  ...
  if (isLoading) return (
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-navy-900 pt-24 pb-10">
        <div className="container mx-auto px-4 max-w-6xl"><Skeleton className="h-12 w-3/4 bg-white/10 mb-4" /></div>
      </div>
      <div className="container mx-auto px-4 max-w-6xl py-12 space-y-4">
  ...
  if (isError || !blog) return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="text-center">
        ...
        <Link to="/blog" className="text-gold-600 font-semibold hover:underline">← Back to Blog</Link>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-navy-900 pt-24 pb-0 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl pb-10">
          <Link to="/blog" className="inline-flex items-center gap-2 text-white/50 hover:text-white text-sm mb-6 transition-colors">
            ...
            <div className="space-y-10 lg:space-y-12">
              {blog.sections?.map((section, i) => (
                <motion.div key={i} id={`section-${i}`}
                  ...
                  viewport={{ once: true }} className="scroll-mt-28">
                  {section.heading && (
                    <h2 className="font-display text-xl sm:text-2xl lg:text-3xl font-extrabold text-navy-900 mb-4 pb-3 border-b-2 border-gold-500/30">
                      {section.heading}
                    </h2>
                  )}
        ...
        <div className="bg-slate-50 py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map(b => <RelatedCard key={b._id} b={b} />)}
            </div>
          </div>
        </div>
      )}
