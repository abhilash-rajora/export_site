...
const ProductCard = React.memo(function ProductCard({ product, hideStockBadge = false }: ProductCardProps) {
  ...
  const handleShare = useCallback((e: React.MouseEvent) => {
    e.preventDefault(); e.stopPropagation();
    const url = `${window.location.origin}/products/detail/${product._id}`;
  ...
  return (
    <Link href={`/products/detail/${product._id}`} className="block h-full">
    <div
      className={cn(
      ...
      <div className="flex flex-col flex-1 p-3">
        ...
        <div className="mt-auto">
          <button disabled={!inStock}
            onClick={e => { e.preventDefault(); e.stopPropagation(); if (!inStock) return; router.push(`/enquiry?productName=${encodeURIComponent(product.name)}`); }}
