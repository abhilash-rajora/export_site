...
export default function ProductDetailPage() {
  ...
  // ✅ FIXED: No <Head> usage — metadata is handled by generateMetadata in the server page
  useEffect(() => {
  ...
  const toggleWishlist = () => {
    ...
    try {
      const saved = JSON.parse(localStorage.getItem('wishlist') ?? '[]') as string[];
      const next = isWishlisted ? saved.filter(v => v !== id) : [...saved, id];
      localStorage.setItem('wishlist', JSON.stringify(next));
      setIsWishlisted(!isWishlisted);
      window.dispatchEvent(new Event('wishlist-updated'));
    } catch { setIsWishlisted(!isWishlisted); }
  };
  ...
  const handleShare = async () => {
    ...
    if (navigator.share) {
      try { await navigator.share({ title, url }); setShareTooltip('shared'); } catch { }
    } else {
      try { await navigator.clipboard.writeText(url); setShareTooltip('copied'); } catch { }
    }
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <AnimatePresence>
        {zoomOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <button className="absolute top-4 right-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white z-10" onClick={() => setZoomOpen(false)}>
              <X className="w-5 h-5" />
            </button>
            {allImages.length > 1 && (
              <>
                <div className="absolute top-4 left-1/2 -translate-x-1/2 text-white/60 text-sm">{zoomImg + 1} / {allImages.length}</div>
                <button className="absolute left-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white z-10"
                  onClick={e => { e.stopPropagation(); setZoomImg(p => (p - 1 + allImages.length) % allImages.length); }}>
                  <ChevronLeft className="w-5 h-5" />
                </button>
              </>
            )}
            <motion.img key={zoomImg} initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} transition={{ duration: 0.2 }}
              src={allImages[zoomImg]} alt={product.name}
              className="max-w-[90vw] max-h-[85vh] object-contain rounded-lg shadow-2xl"
              onClick={e => e.stopPropagation()} />
            {allImages.length > 1 && (
              <>
                <button className="absolute right-4 w-10 h-10 rounded-full bg-white/10 hover:bg-white/20 flex items-center justify-center text-white z-10"
                  onClick={e => { e.stopPropagation(); setZoomImg(p => (p + 1) % allImages.length); }}>
                  <ChevronRight className="w-5 h-5" />
                </button>
                <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex gap-2">
                  {allImages.map((img: string, i: number) => (
                    <button key={i} onClick={e => { e.stopPropagation(); setZoomImg(i); }}
      ...
      <div className="bg-navy-900 pt-24 pb-10">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
          <Link href="/products" className="inline-flex items-center gap-2 text-sm text-white/60 hover:text-white transition-colors">
            <ChevronLeft className="w-4 h-4" />Back to Products
          </Link>
          <h1 className="font-display text-xl sm:text-2xl font-extrabold text-white tracking-tight leading-tight mt-4">{product.name}</h1>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-12 max-w-6xl">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}>
            <div className="relative h-80 sm:h-96 bg-muted rounded-2xl overflow-hidden mb-3 shadow-xl cursor-zoom-in group hover:shadow-[0_20px_40px_rgba(0,0,0,0.3)]"
              onClick={() => allImages.length > 0 && (() => { setZoomImg(activeImg); setZoomOpen(true); })()}
              ...
              {allImages.length > 0 ? (
                <>
                  ...
                  <div className="absolute bottom-2 right-2 bg-black/40 text-white text-xs px-2 py-1 rounded-full flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                    <ZoomIn className="w-3 h-3" /><span>Click to zoom</span>
                  </div>
                  {allImages.length > 1 && (
                    <>
                ...
                <div className="w-full h-full flex items-center justify-center"><Package className="w-20 h-20 text-muted-foreground/20" /></div>
              )}
            </div>
              ...
              <div className="flex gap-2 mb-6 overflow-x-auto pb-1">
                {allImages.map((img: string, i: number) => (
                  <button key={i} onClick={() => setActiveImg(i)}
                    className={`flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 transition-all ${i === activeImg ? 'border-gold-500' : 'border-transparent opacity-60 hover:opacity-100'}`}>
            ...
            <div className="flex gap-2 mb-3">
              <motion.button whileTap={{ scale: 0.94 }} onClick={toggleWishlist}
                className={`flex-1 flex items-center justify-center gap-2 py-2.5 rounded-lg border text-sm font-medium transition-all duration-200 ${isWishlisted ? 'bg-rose-50 border-rose-200 text-rose-600 hover:bg-rose-100' : 'bg-white border-border text-muted-foreground hover:border-rose-200 hover:text-rose-500 hover:bg-rose-50'}`}>
                <AnimatePresence mode="wait">
                  <motion.span key={isWishlisted ? 'filled' : 'empty'} initial={{ scale: 0.5, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.5, opacity: 0 }} transition={{ duration: 0.15 }} className="flex items-center">
              ...
              <div className="relative">
                ...
                <AnimatePresence>
                  {shareTooltip !== 'idle' && (
                    <motion.div initial={{ opacity: 0, y: 6, scale: 0.9 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 6, scale: 0.9 }} transition={{ duration: 0.15 }}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="space-y-6 sticky top-24">
            <div className="flex flex-wrap gap-2 mb-2">
              <span className="px-3 py-1 text-sm font-medium rounded-full bg-green-100 text-green-700">✔ Verified Supplier</span>
              <span className="px-3 py-1 text-sm font-medium rounded-full bg-blue-100 text-blue-700">🌍 Export Quality</span>
              <span className="px-3 py-1 text-sm font-medium rounded-full bg-orange-100 text-orange-700">⏱ 24h Response</span>
            </div>
            <div className="bg-card rounded-xl border border-border p-6">
            ...
            <div className="bg-card rounded-2xl border border-border p-6 border-l-4 border-gold-500 shadow-lg hover:shadow-xl transition-shadow duration-300">
              <div className="mb-6">
                ...
                <p className="text-xs text-green-600 font-medium mt-2">✔ No minimum enquiry • Quick response guaranteed</p>
              </div>
