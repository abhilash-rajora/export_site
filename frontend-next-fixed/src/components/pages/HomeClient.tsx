...
import { ArrowRight, ChevronLeft, ChevronRight, Coffee, Cpu, Gem, Globe, Leaf, Palette, Shield, Shirt, Star, Truck, MapPin } from 'lucide-react';
import { motion } from 'framer-motion';
import React, { useRef, useCallback } from 'react';
import CategoryCard3D from '@/components/3d/CategoryCard3D';
import type { HomepageData, HomeProduct } from '@/hooks/useHomepage';
import { useRouter } from "next/navigation";


const categories = [
  { name: 'Agriculture',      slug: 'agriculture',   icon: Leaf,    desc: 'Fresh produce & farm goods',  color: 'bg-green-50 text-green-700 border-green-200', image: 'https://images.unsplash.com/photo-1464226184884-fa280b87c399?w=400&q=70' },
  { name: 'Textiles',         slug: 'textiles',       icon: Shirt,   desc: 'Fabrics & garments',           color: 'bg-purple-50 text-purple-700 border-purple-200', image: 'https://images.unsplash.com/photo-1558769132-cb1aea458c5e?w=400&q=70' },
  { name: 'Minerals',         slug: 'minerals',       icon: Gem,     desc: 'Raw minerals & ores',          color: 'bg-stone-50 text-stone-700 border-stone-200', image: 'https://www.sreemetaliks.com/blog/public/assets/images/blog/blog-2-Hematite_1707918282.webp' },
  { name: 'Electronics',      slug: 'electronics',    icon: Cpu,     desc: 'Components & devices',         color: 'bg-blue-50 text-blue-700 border-blue-200', image: 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=400&q=70' },
  { name: 'Food & Beverages', slug: 'food-beverages', icon: Coffee,  desc: 'Packaged foods & drinks',      color: 'bg-orange-50 text-orange-700 border-orange-200', image: 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=400&q=70' },
  { name: 'Handicrafts',      slug: 'handicrafts',    icon: Palette, desc: 'Artisan & handmade goods',     color: 'bg-amber-50 text-amber-700 border-amber-200', image: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=400&q=70' },
];
    ...
    <>
      ...
      {showEnquiry && (
        <div className="px-3 pb-3 pt-1.5">
            <button
              className="w-full h-9 text-xs font-black uppercase tracking-wide text-navy-900 hover:opacity-90 transition-all"
              style={{ background: '#D4A017', borderRadius: '8px' }}
              onClick={(e) => {
                e.stopPropagation();
                e.preventDefault();      // ⭐ IMPORTANT
                router.push(`/enquiry?productName=${product.name}`);
              }}
            >
              Get Price
            </button>
...
const FeaturedCard = React.memo(function FeaturedCard({ product, badge, badgeColor, eager = false }: { product: HomeProduct; badge?: string; badgeColor?: string; eager?: boolean }) {
  return (
    <Link href={`/products/detail/${product._id}`} className="block flex-shrink-0 w-[270px] sm:w-[360px]">
...
const ArrivalSliderCard = React.memo(function ArrivalSliderCard({ product }: { product: HomeProduct }) {
  return (
    <Link href={`/products/detail/${product._id}`} className="block flex-shrink-0 w-[155px] sm:w-[185px]">
      <motion.div whileHover={{ y: -4 }} transition={{ type: 'spring', stiffness: 280, damping: 22 }}
        className="group flex flex-col rounded-2xl overflow-hidden bg-white w-full h-full"
        style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.1)' }}>
...
const desktopCollage = [
  { slug: 'agriculture',    src: 'https://images.unsplash.com/photo-1464226184884-fa280b87c399?w=600&q=80', label: 'Fresh Produce & Farm Goods', span: 'col-span-4 row-span-2' },
  { slug: 'textiles',       src: 'https://images.unsplash.com/photo-1558769132-cb1aea458c5e?w=600&q=80',    label: 'Fabrics & Garments',        span: 'col-span-4 row-span-1' },
  { slug: 'electronics',    src: 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=600&q=80', label: 'Components & Devices',       span: 'col-span-4 row-span-1' },
  { slug: 'minerals',       src: 'https://www.sreemetaliks.com/blog/public/assets/images/blog/blog-2-Hematite_1707918282.webp', label: 'Raw Ores', span: 'col-span-2 row-span-1' },
  { slug: 'food-beverages', src: 'https://images.unsplash.com/photo-1542838132-92c53300491e?w=500&q=80',    label: 'Packaged Foods',             span: 'col-span-3 row-span-1' },
  { slug: 'handicrafts',    src: 'https://images.unsplash.com/photo-1600585154340-be6161a56a0c?w=500&q=80', label: 'Artisan Goods',              span: 'col-span-3 row-span-1' },
];

export default function HomeClient({ initialData }: { initialData: HomepageData }) {
  // ✅ FIXED: Use structured data from /products/homepage (not flat array)
  const featured   = initialData?.featured    ?? [];
  const newArrivals = initialData?.newArrivals ?? [];
      ...
      <section className="py-10 sm:py-16 bg-[#0D3D3D]">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between mb-6">
            ...
            <Link href="/products" className="flex items-center gap-1 text-xs font-bold text-white/40 hover:text-gold-400 transition-colors">
          ...
          <div className="grid sm:hidden grid-cols-3 gap-2" style={{ height: '220px' }}>
            {categories.map(cat => (
              <Link key={cat.slug} href={`/products/${cat.slug}`}
                ...
                <img src={cat.image} alt={cat.name} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" decoding="async" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/75 to-transparent" />
                <div className="absolute bottom-1.5 left-2">
                  <p className="font-display font-bold text-white/80 text-[10px] leading-tight">{cat.name}</p>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="hidden sm:grid grid-cols-12 grid-rows-2 gap-3" style={{ height: '380px' }}>
            {desktopCollage.map(cat => (
              <Link key={cat.slug} href={`/products/${cat.slug}`}
                className={`${cat.span} relative rounded-2xl overflow-hidden group cursor-pointer`}>
                <img src={cat.src} alt={cat.label} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" loading="lazy" decoding="async" />
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent" />
                <div className="absolute bottom-3 left-3">
                  <h3 className="font-display font-bold text-white/80 text-sm">{cat.label}</h3>
                </div>
              </Link>
            ))}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-12 items-start">
            <div className="lg:col-span-4">
              <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
                ...
                <Link href="/products" className="inline-flex items-center gap-2 mt-5 text-xs font-black uppercase tracking-widest text-gold-400 hover:text-gold-300 transition-colors group">
            ...
            <div className="lg:col-span-8">
              {!trending.length ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 items-stretch">
                  {trending.slice(0, 4).map((p, i) => <TrendingCard key={p._id} product={p} index={i} />)}
                </div>
              )}
            </div>
          </div>
