...
import {
  Calendar, Clock, Tag, ChevronLeft, ChevronRight, Share2, Facebook, Linkedin,
  ArrowRight, BookOpen, Eye, User, Heart, ChevronDown
} from 'lucide-react';
import { FaWhatsapp } from 'react-icons/fa';
import { usePublishedBlogs, Blog } from '@/hooks/useBlogs';
...
const MediaSlider = memo(function MediaSlider({ images, videoUrl, heading }: {
  images: string[]; videoUrl?: string; heading?: string;
}) {
  ...
  const prev = useCallback(() => setActive(p => (p - 1 + all.length) % all.length), [all.length]);
  const next = useCallback(() => setActive(p => (p + 1) % all.length), [all.length]);
  return (
      ...
      <div className="relative aspect-video">
        {isVid(all[active]) ? (
          all[active].includes('youtube') || all[active].includes('youtu.be')
            ? <iframe src={embed(all[active])} className="w-full h-full" allowFullScreen title={heading} loading="lazy" />
            : <video controls className="w-full h-full bg-black"><source src={all[active]} /></video>
        ) : (
          <img src={all[active]} alt={heading ?? ''} loading="lazy" decoding="async" className="w-full h-full object-cover" />
        )}
        {all.length > 1 && (
          <>
            <button onClick={prev} className="absolute left-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center"><ChevronLeft className="w-4 h-4" /></button>
            <button onClick={next} className="absolute right-3 top-1/2 -translate-y-1/2 w-9 h-9 rounded-full bg-black/50 hover:bg-black/70 text-white flex items-center justify-center"><ChevronRight className="w-4 h-4" /></button>
            <div className="absolute bottom-3 left-1/2 -translate-x-1/2 flex gap-1.5">
              {all.map((_, i) => (
                <button key={i} onClick={() => setActive(i)} className={`h-2 rounded-full transition-all ${i === active ? 'bg-gold-500 w-5' : 'bg-white/50 w-2'}`} />
              ))}
            </div>
          </>
        )}
...
const ShareButtons = memo(function ShareButtons({ url, title }: { url: string; title: string }) {
  const [copied, setCopied] = useState(false);
  const share = useCallback((p: string) => {
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      ...
      <button onClick={copyLink} className="flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-gray-100 text-gray-600 hover:bg-gray-200 text-xs font-semibold transition-colors">
...
const LikeButton = memo(function LikeButton({ blogId }: { blogId: string }) {
...
const MobileTOC = memo(function MobileTOC({ sections }: { sections: { heading?: string }[] }) {
  ...
  return (
    <div className="lg:hidden sticky top-16 z-30 mb-4">
      <button onClick={() => setOpen(v => !v)}
        className="w-full flex items-center justify-between px-4 py-3 backdrop-blur-md bg-white/60 border border-gold-200 rounded-xl text-sm font-semibold text-navy-900 shadow-sm">
        <span className="flex items-center gap-2">📋 Table of Contents</span>
        <motion.span animate={{ rotate: open ? 180 : 0 }} transition={{ duration: 0.2 }}><ChevronDown className="w-4 h-4 text-gold-600" /></motion.span>
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}
            ...
            <ol className="space-y-1">
              {headings.map((s, i) => (
                <li key={i}>
                  <a href={`#section-${i}`} onClick={() => setOpen(false)}
...
const StickyTOC = memo(function StickyTOC({ sections }: { sections: { heading?: string }[] }) {
  const headings = sections.filter(s => s.heading?.trim());
  if (headings.length === 0) return null;
  return (
    <div className="bg-gold-50 border border-gold-200 rounded-2xl p-5">
      <h3 className="font-display font-bold text-navy-900 text-sm uppercase tracking-widest mb-4">📋 Table of Contents</h3>
      <ol className="space-y-2">
        {headings.map((s, i) => (
          <li key={i}>
            <a href={`#section-${i}`} className="flex items-start gap-2 text-sm text-navy-700 hover:text-gold-600 transition-colors">
              <span className="text-gold-500 font-bold flex-shrink-0">{i + 1}.</span>
              <span className="leading-snug">{s.heading}</span>
            </a>
          </li>
...
const RelatedCard = memo(function RelatedCard({ b }: { b: Blog }) {
  return (
    <Link href={`/blog/${b.slug}`}>
      <div className="group bg-white rounded-2xl overflow-hidden border border-gray-100 hover:shadow-lg transition-all">
        {b.coverImage && (
          <div className="h-40 overflow-hidden">
            <img src={b.coverImage} alt={b.title} loading="lazy" decoding="async" width={400} height={160}
        ...
        <div className="p-5">
          ...
          <span className="text-xs text-gray-400">{new Date(b.publishedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
        </div>
      </div>
    </Link>
  );
...
// ✅ FIXED: RecentPost had broken layout (missing flex container)
const RecentPost = memo(function RecentPost({ post }: { post: Blog }) {
  return (
    <Link href={`/blog/${post.slug}`} className="flex gap-3 group">
      <div className="w-14 h-14 rounded-lg overflow-hidden flex-shrink-0 bg-navy-900">
        {post.coverImage
          ? <img src={post.coverImage} alt={post.title} loading="lazy" decoding="async" width={56} height={56} className="w-full h-full object-cover group-hover:scale-105 transition-transform" />
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[11px] text-gray-400 mt-1">{new Date(post.publishedAt).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
      </div>
    </Link>
  );
});

export default function BlogDetailPage({ initialData, slug }: { initialData: any; slug: string }) {
  const blog = initialData?.blog;
  const related = initialData?.related ?? [];

  // ✅ FIXED: pageUrl via useEffect to avoid SSR mismatch
  const [pageUrl, setPageUrl] = useState('');
  useEffect(() => { setPageUrl(window.location.href); }, []);

  const { data: recentData } = usePublishedBlogs({ sort: 'latest' });
  const recentPosts = recentData?.blogs?.filter((b: Blog) => b.slug !== slug).slice(0, 4) ?? [];

  // ✅ FIXED: removed undefined isLoading/isError references — data comes from server via props
  if (!blog) return (
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="text-center">
        <BookOpen className="w-16 h-16 text-gray-200 mx-auto mb-4" />
        <h2 className="font-display font-bold text-2xl text-navy-900 mb-4">Article not found</h2>
        <Link href="/blog" className="text-gold-600 font-semibold hover:underline">← Back to Blog</Link>
      </div>
    </div>
  );
            ...
            {blog.tags?.length > 0 && (
              <div className="flex flex-wrap gap-2 mt-5">
                {blog.tags.map((tag: string) => (
                  <span key={tag} className="flex items-center gap-1 text-xs text-white/40 bg-white/5 border border-white/10 px-3 py-1 rounded-full">
        ...
        {blog.coverImage && (
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              className="rounded-t-3xl overflow-hidden h-72 sm:h-[420px]">
              <img src={blog.coverImage} alt={blog.title} loading="eager" decoding="async" width={1200} height={420} className="w-full h-full object-cover" />
            </motion.div>
          </div>
        )}
      </div>
          ...
          <div className="lg:col-span-8 min-w-0">
            ...
            <div className="space-y-10 lg:space-y-12">
              {blog.sections?.map((section: any, i: number) => (
                <motion.div key={i} id={`section-${i}`} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="scroll-mt-28">
                  {section.heading && (
                    <h2 className="font-display text-xl sm:text-2xl lg:text-3xl font-extrabold text-navy-900 mb-4 pb-3 border-b-2 border-gold-500/30">{section.heading}</h2>
                  )}
            ...
            {blog.tags?.length > 0 && (
              <div className="mt-10 pt-8 border-t border-gray-100 flex flex-wrap gap-2">
                ...
                {blog.tags.map((tag: string) => <span key={tag} className="text-xs bg-gray-100 text-gray-600 px-3 py-1 rounded-full">#{tag}</span>)}
              </div>
            )}
            ...
            <div className="mt-8 p-5 sm:p-6 bg-navy-900 rounded-2xl">
              ...
              <div className="flex flex-wrap gap-3">
                <Link href="/enquiry"><Button className="bg-gold-500 hover:bg-gold-400 text-navy-900 font-bold">Get Quote in 24hrs</Button></Link>
                <Link href="/products"><Button variant="outline" className="border-white/30 text-white hover:bg-white/10">Browse Products</Button></Link>
              </div>
            </div>
          </div>
            ...
            <div className="sticky top-24 space-y-5">
              ...
              {recentPosts.length > 0 && (
                <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4">
                  ...
                  <div className="space-y-3">
                    {recentPosts.map((post: Blog) => <RecentPost key={post._id} post={post} />)}
                  </div>
                </div>
              )}
              <div className="bg-gold-500 rounded-2xl p-4 text-navy-900">
                ...
                <Link href="/enquiry">
                  <Button className="w-full bg-navy-900 hover:bg-navy-800 text-white font-bold text-sm">Contact Us Now →</Button>
                </Link>
              </div>
            </div>
          </aside>
        ...
        <div className="bg-slate-50 py-16">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-6xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {related.map((b: Blog) => <RelatedCard key={b._id} b={b} />)}
            </div>
          </div>
        </div>
      )}
