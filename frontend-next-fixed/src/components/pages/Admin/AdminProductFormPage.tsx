...
const defaultForm = {
  name: '', category: '', description: '', imageUrl: '', images: [] as string[],
  specifications: [{ property: '', value: '' }],
  originCountry: '', minOrderQty: '', priceRange: '', inStock: true,
};

export default function AdminProductFormPage({ id }: { id?: string }) {
  ...
  useEffect(() => {
    if (isEdit && product) {
      setForm({
        ...
        originCountry: product.originCountry, minOrderQty: String(product.minOrderQty),
        priceRange: product.priceRange, inStock: product.inStock !== false,
      });
    }
  }, [isEdit, product]);
  ...
  const handleFileUpload = async (file: File, index: number) => {
    if (!file) return;
    if (!['image/jpeg', 'image/jpg', 'image/png', 'image/webp'].includes(file.type)) { toast.error('Only JPG, PNG, or WebP images allowed.'); return; }
    ...
    try {
      ...
      setForm((prev) => {
        ...
        if (index < updated.length) { updated[index] = uploadedUrl; } else { updated.push(uploadedUrl); }
  ...
  const handleAddUrl = () => {
    ...
    setPendingUrl(''); setUrlMode(false);
  };
  ...
  const addSpecRow = () => setForm((prev) => ({ ...prev, specifications: [...prev.specifications, { property: '', value: '' }] }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const data = { name: form.name, category: form.category, description: form.description, imageUrl: form.images[0] || '', images: form.images, specifications: cleanSpecs, originCountry: form.originCountry, minOrderQty: Number(form.minOrderQty), priceRange: form.priceRange, inStock: form.inStock };
    try {
      if (isEdit && id) { await updateProduct.mutateAsync({ id, ...data }); toast.success('Product updated successfully.'); }
      else { await createProduct.mutateAsync(data); toast.success('Product created successfully.'); }
      router.push('/admin/products');
    } catch { toast.error(isEdit ? 'Failed to update product.' : 'Failed to create product.'); }
  };

  const isPending = createProduct.isPending || updateProduct.isPending;

  if (isEdit && isLoadingProduct) {
    return <div className="max-w-2xl space-y-4"><Skeleton className="h-8 w-48 bg-white/10" /><Skeleton className="h-96 rounded-xl bg-white/5" /></div>;
  }

  return (
    <div className="max-w-2xl">
      <div className="mb-8">
        <Link href="/admin/products" className="inline-flex items-center gap-2 text-sidebar-foreground/50 hover:text-sidebar-foreground transition-colors text-sm mb-4">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white/5 rounded-xl border border-sidebar-border p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              ...
              <Select value={form.category} onValueChange={(v) => setForm((p) => ({ ...p, category: v }))}>
                <SelectTrigger className="bg-white/5 border-sidebar-border text-sidebar-foreground"><SelectValue placeholder="Select category" /></SelectTrigger>
          ...
          <div className="space-y-1.5">
            ...
            <Textarea name="description" placeholder={`Premium quality product\nAvailable in bulk quantities`} value={form.description} onChange={handleChange} rows={5} required className="bg-white/5 border-sidebar-border text-sidebar-foreground placeholder:text-sidebar-foreground/30 font-mono text-sm" />
          </div>
              ...
              {form.specifications.map((spec, index) => (
                <div key={index} className="grid grid-cols-[1fr_1fr_auto] border-b border-sidebar-border/50 last:border-b-0">
                  ...
                  <div className="flex items-center justify-center w-10">
                    <button type="button" onClick={() => removeSpecRow(index)} className="text-sidebar-foreground/30 hover:text-destructive transition-colors p-1"><Trash2 className="w-3.5 h-3.5" /></button>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {form.images.map((url, index) => (
                <div key={index} className="relative group aspect-square rounded-xl overflow-hidden bg-white/5 border border-sidebar-border">
                  ...
                  <button type="button" onClick={() => removeImage(index)} className="absolute top-1.5 right-1.5 w-6 h-6 rounded-full bg-black/60 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-500"><X className="w-3 h-3" /></button>
                  {index === 0 && <div className="absolute bottom-1.5 left-1.5 bg-gold-500 text-navy-900 text-[10px] font-bold px-1.5 py-0.5 rounded-md">Cover</div>}
                </div>
              ))}
              {form.images.length < 5 && (
                <div className={`rounded-2xl border-2 border-dashed border-sidebar-border flex flex-col overflow-hidden ${form.images.length === 0 ? 'col-span-3 h-48' : 'aspect-square'}`}>
                  <div className="flex border-b border-sidebar-border flex-shrink-0">
                    <button type="button" onClick={() => setUrlMode(false)} className={`flex-1 text-xs font-semibold py-2 ${!urlMode ? 'bg-white/10 text-gold-400' : 'text-sidebar-foreground/30'}`}>Upload</button>
                    <button type="button" onClick={() => setUrlMode(true)} className={`flex-1 text-xs font-semibold py-2 ${urlMode ? 'bg-white/10 text-gold-400' : 'text-sidebar-foreground/30'}`}>URL</button>
                  </div>
                  {!urlMode ? (
                    <button type="button" onClick={() => fileInputRefs.current[form.images.length]?.click()} disabled={uploadingIndex !== null} className="flex-1 flex flex-col items-center justify-center gap-3 text-sidebar-foreground/40 hover:text-gold-400 hover:bg-white/5 transition-all">
                      {uploadingIndex === form.images.length ? <><Loader2 className="w-8 h-8 animate-spin text-gold-400" /><span className="text-xs">Uploading...</span></> : <><ImagePlus className="w-6 h-6" /><span className="text-sm font-semibold">Click to upload</span></>}
                    </button>
                  ) : (
                    <div className="flex-1 flex flex-col items-center justify-center gap-3 p-5">
                      <input type="url" placeholder="https://example.com/image.jpg" value={pendingUrl} onChange={(e) => setPendingUrl(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddUrl(); } }} className="w-full bg-white/5 border border-sidebar-border rounded-xl px-3 py-2 text-sm text-sidebar-foreground placeholder:text-sidebar-foreground/30 focus:outline-none focus:border-gold-400" />
                      <button type="button" onClick={handleAddUrl} className="w-full bg-gold-500 hover:bg-gold-400 text-navy-900 text-sm font-bold py-2 rounded-xl">Add Image</button>
                    </div>
                  )}
                </div>
              )}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label className="text-sidebar-foreground text-sm font-medium">Min. Order Qty <span className="text-destructive">*</span></Label>
          ...
          <div className="flex items-center justify-between p-4 bg-white/5 rounded-xl border border-sidebar-border">
            ...
            <button type="button" onClick={() => setForm((p) => ({ ...p, inStock: !p.inStock }))}
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-sm font-bold transition-all ${form.inStock ? 'bg-green-500/20 text-green-400 border border-green-500/30' : 'bg-red-500/20 text-red-400 border border-red-500/30'}`}>
          ...
          <div className="flex items-center gap-3 pt-2">
            <Button type="submit" disabled={isPending} className="bg-gold-500 hover:bg-gold-400 text-navy-900 font-bold shadow-gold">
              {isPending ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />{isEdit ? 'Updating...' : 'Creating...'}</> : <><Save className="mr-2 h-4 w-4" />{isEdit ? 'Save Changes' : 'Create Product'}</>}
            </Button>
            <Button type="button" variant="ghost" onClick={() => router.push('/admin/products')} className="text-sidebar-foreground/60 hover:text-sidebar-foreground hover:bg-white/5">Cancel</Button>
          </div>
        </form>
      </motion.div>
    </div>
